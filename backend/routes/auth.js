...
router.post('/alumni', [
  ...
  try {
    ...
    if (user) {
      success = false;
      return res.status(400).json({ errors: [{ param: "email", msg: "Email is already in use" }] })
    ...
    if (req.file) {
      newAlumni.profileImage = req.file.buffer; // Store the image data as a buffer
      newAlumni.imageType = req.file.mimetype; // Store the image type
    }
    const data = {
      newAlumni: {
        id: newAlumni.id
